...
var fiveWordsArray = ['chocolate', 'strawberry', 'vanilla', 'cookies', 'cream'];
for(var i=0; i<5; i++){
  alert(message = fiveWordsArray[i]);
}
...
for(var i=0; i<5; i++){
  alert(message = fiveWordsArray[i].substring(0,1).toUpperCase() + fiveWordsArray[i].substring(1));
}
...
var sentence = '';
for(var i=0; i<5; i++){
  sentence = sentence + fiveWordsArray[i] + ' ';
}
  alert(sentence);
